...
**动这段代码前请注意**：把 `root` 去掉（退回系统临时目录）不会报任何错，但会让
上面那句提示语变成谎言 —— 模型照着 read_file 会吃到围栏拒绝，只能改用 run_bash，
而每条 run_bash 都要用户批一次。症状是"怎么老要审批"，离原因很远。
想复核：跑一条输出超过 BASH_OUTPUT_LIMIT 的命令，按定位符里的路径在
workspace-write 下 read_file 读一次，应该读得到。
（这条不变式原来有一个专门的校验脚本，后来按"少放文件"的要求删了；复核方法
留在这里，免得下次再踩。）
"""
